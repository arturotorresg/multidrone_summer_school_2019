...
////////////////////////////////////////////////////////////////////////////////
void DRCVehicleROSPlugin::SetWaypointState(const geometry_msgs::Point::ConstPtr &_msg)
{
  math::Vector3 wp = math::Vector3(_msg->x, _msg->y, 1.0);
  DRCVehiclePlugin::SetWaypointState(wp);
}

////////////////////////////////////////////////////////////////////////////////
void DRCVehicleROSPlugin::SetSpeedState(const std_msgs::Float64::ConstPtr &_msg)
{
  DRCVehiclePlugin::SetSpeedState(static_cast<double>(std::min(_msg->data, 2.0)));
}
...
////////////////////////////////////////////////////////////////////////////////
nav_msgs::Odometry DRCVehicleROSPlugin::GetOdometry()
{
  nav_msgs::Odometry odom;
  math::Pose pose = DRCVehiclePlugin::GetPose();
  math::Vector3 linVel = DRCVehiclePlugin::GetLinVel();
  math::Vector3 angVel = DRCVehiclePlugin::GetAngVel();
...
////////////////////////////////////////////////////////////////////////////////
void DRCVehicleROSPlugin::SetHandBrakePercent(double _percent)
{
  double min, max, percent, cmd;
  percent = math::clamp(_percent, 0.0, 1.0);
...
void DRCVehicleROSPlugin::SetHandBrakePercent(const std_msgs::Float64::ConstPtr
    &_msg)
{
  SetHandBrakePercent(static_cast<double>(_msg->data));
}
                                 ...
                                 sdf::ElementPtr _sdf)
{
  ...
  if (cheatsEnabledString && (std::string(cheatsEnabledString) == "0"))
  ...
  {
    //ros::SubscribeOptions waypoint_cmd_so =
    //  ros::SubscribeOptions::create<geometry_msgs::Point>(
    //  this->model->GetName() + "/waypoint/cmd", 100,
    //  boost::bind(static_cast< void (DRCVehicleROSPlugin::*)
    //    (const geometry_msgs::Point::ConstPtr&) >(
    //      &DRCVehicleROSPlugin::SetWaypointState), this, _1),
    //  ros::VoidPtr(), &this->queue);
    //this->subWaypointCmd = this->rosNode->subscribe(waypoint_cmd_so);

    ros::SubscribeOptions speed_cmd_so =
      ros::SubscribeOptions::create<std_msgs::Float64>(
      this->model->GetName() + "/speed/cmd", 100,
      boost::bind(static_cast< void (DRCVehicleROSPlugin::*)
        (const std_msgs::Float64::ConstPtr&) >(
          &DRCVehicleROSPlugin::SetSpeedState), this, _1),
      ros::VoidPtr(), &this->queue);
    this->subSpeedCmd = this->rosNode->subscribe(speed_cmd_so);
  ...
  */
    this->pubOdometry = this->rosNode->advertise<nav_msgs::Odometry>(
      this->model->GetName() + "/odometry", 10);
    //this->pubHeadingState = this->rosNode->advertise<std_msgs::Float64>(
    //  this->model->GetName() + "/heading", 10);
    //this->pubHeadingError = this->rosNode->advertise<std_msgs::Float64>(
    //  this->model->GetName() + "/heading/error", 10);
    //this->pubCrossTrackState = this->rosNode->advertise<std_msgs::Float64>(
    //  this->model->GetName() + "/cross_track/state", 10);
    //this->pubCrossTrackError = this->rosNode->advertise<std_msgs::Float64>(
    //  this->model->GetName() + "/cross_track/error", 10);
    //this->pubSpeedState = this->rosNode->advertise<std_msgs::Float64>(
    //  this->model->GetName() + "/speed/state", 10);
...
void DRCVehicleROSPlugin::RosPublishStates()
{
      ...
      this->rosPublishPeriod)
  {
    ...
    std_msgs::Float64 msg_steer, msg_brake, msg_gas, msg_hand_brake, msg_speed;
    std_msgs::Float64 msg_heading, msg_crossTrack, msg_heading_error, msg_crossTrack_error;
    nav_msgs::Odometry msg_odometry;

    msg_odometry = DRCVehicleROSPlugin::GetOdometry();
    this->pubOdometry.publish(msg_odometry);
    //msg_heading.data = GetHeadingState();
    //this->pubHeadingState.publish(msg_heading);
    //msg_heading_error.data = GetHeadingError();
    //this->pubHeadingError.publish(msg_heading_error);
    //msg_crossTrack.data = GetCrossTrackState();
    //this->pubCrossTrackState.publish(msg_crossTrack);
    //msg_crossTrack_error.data = GetCrossTrackError();
    //this->pubCrossTrackError.publish(msg_crossTrack_error);
    //msg_speed.data = GetSpeedState();
    //this->pubSpeedState.publish(msg_speed);
    ...
    //nav_msgs::Odometry msg_odometry;
    //math::Pose pose = this->model->gazebo::physics::ModelState::GetPose();
    //msg_odometry.pose.position = pose._pos;
    //msg_odometry.pose.orientation = pose._rot;
    //this->pubOdometry
  }
}
