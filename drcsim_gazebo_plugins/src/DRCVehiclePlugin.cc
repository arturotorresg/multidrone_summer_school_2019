...
{
  this->pose = math::Pose::Zero;
  this->linVel = math::Vector3::Zero;
  this->keyState = ON;
  this->directionState = FORWARD;
  this->headingCmd = 0.0;
  this->waypointCmd = math::Vector3::UnitZ;
  this->prev_waypointCmd = math::Vector3::UnitZ;
  this->loopWP = false;
  this->indexWP = 0;
  this->directionWP = 1;
  this->speedCmd = 0.0;
  ...
  this->speedCtrlPgain = 0;
  this->speedCtrlIgain = 0;
  this->speedCtrlDgain = 0;
  this->headingPgain = 0;
  this->headingIgain = 0;
  this->headingDgain = 0;
  this->crossTrackPgain = 0;
  this->crossTrackIgain = 0;
  this->crossTrackDgain = 0;
}
                                   ...
                                   DRCVehiclePlugin::DirectionType _direction)
{
  ...
  this->keyState = _key;
  this->cruiseSpeed = _speed;
  //this->waypointCmd = _waypoint;
  this->listWP.push_back(_waypoint);
}

////////////////////////////////////////////////////////////////////////////////
math::Pose DRCVehiclePlugin::GetPose()
{
  return this->pose;
}

////////////////////////////////////////////////////////////////////////////////
math::Vector3 DRCVehiclePlugin::GetLinVel()
{
  return this->linVel;
}

////////////////////////////////////////////////////////////////////////////////
math::Vector3 DRCVehiclePlugin::GetAngVel()
{
  return this->angVel;
}

////////////////////////////////////////////////////////////////////////////////
void DRCVehiclePlugin::SetWaypointState(math::Vector3 _waypoint)
{
    this->listWP.push_back(_waypoint);
}
...
////////////////////////////////////////////////////////////////////////////////
double DRCVehiclePlugin::GetSpeedState()
{
  return this->speedState;
}

////////////////////////////////////////////////////////////////////////////////
double DRCVehiclePlugin::GetHeadingState()
{
  return this->headingState;
}

////////////////////////////////////////////////////////////////////////////////
double DRCVehiclePlugin::GetHeadingError()
{
  return this->headingError;
}

////////////////////////////////////////////////////////////////////////////////
double DRCVehiclePlugin::GetCrossTrackState()
{
  return this->crossTrackState;
}

////////////////////////////////////////////////////////////////////////////////
double DRCVehiclePlugin::GetCrossTrackError()
{
  return this->crossTrackError;
}

////////////////////////////////////////////////////////////////////////////////
void DRCVehiclePlugin::SetSpeedState(double _speed)
{
  this->cruiseSpeed = _speed;
}
                                 ...
                                 sdf::ElementPtr _sdf)
{
  ...
  // get PID values
  paramName = "speed_ctrl_p_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->speedCtrlPgain = _sdf->Get<double>(paramName);
  else
    this->speedCtrlPgain = paramDefault;

  paramName = "speed_ctrl_i_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->speedCtrlIgain = _sdf->Get<double>(paramName);
  else
    this->speedCtrlIgain = paramDefault;

  paramName = "speed_ctrl_d_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->speedCtrlDgain = _sdf->Get<double>(paramName);
  else
    this->speedCtrlDgain = paramDefault;

  paramName = "heading_ctrl_p_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->headingPgain = _sdf->Get<double>(paramName);
  else
    this->headingPgain = paramDefault;

  paramName = "heading_ctrl_i_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->headingIgain = _sdf->Get<double>(paramName);
  else
    this->headingIgain = paramDefault;

  paramName = "heading_ctrl_d_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->headingDgain = _sdf->Get<double>(paramName);
  else
    this->headingDgain = paramDefault;

  paramName = "crosstrack_ctrl_p_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->crossTrackPgain = _sdf->Get<double>(paramName);
  else
    this->crossTrackPgain = paramDefault;

  paramName = "crosstrack_ctrl_i_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->crossTrackIgain = _sdf->Get<double>(paramName);
  else
    this->crossTrackIgain = paramDefault;

  paramName = "crosstrack_ctrl_d_gain";
  paramDefault = 0;
  if (_sdf->HasElement(paramName))
    this->crossTrackDgain = _sdf->Get<double>(paramName);
  else
    this->crossTrackDgain = paramDefault;

  // get waypoints
  paramName = "waypoint";
  int wp_counter = 0;
  while(_sdf->HasElement(std::string(paramName + std::to_string(++wp_counter))))
    this->listWP.push_back(_sdf->Get<math::Vector3>(std::string(paramName + std::to_string(wp_counter))));

  paramName = "loop";
  bool paramBoolDefault = true;
  if(_sdf->GetElement(paramName))
    this->loopWP = _sdf->Get<bool>(paramName);
  else
    this->loopWP = paramBoolDefault;
  ...
  this->crossTrackPID.Init(this->crossTrackPgain,
                            this->crossTrackIgain,
                            this->crossTrackDgain,
                            0, 0,
                            10, -10);
  this->headingPID.Init(this->headingPgain,
                        this->headingIgain,
                        this->headingDgain,
                        0, 0,
                        M_PI, -M_PI);
  this->speedCtrlPID.Init(this->speedCtrlPgain,
                          this->speedCtrlIgain,
                          this->speedCtrlDgain,
                          0, 0,
...
////////////////////////////////////////////////////////////////////////////////
math::Vector3 DRCVehiclePlugin::GetNextWaypoint()
{
    if(this->listWP.size()-this->indexWP == 1)
      this->directionWP = -1;
    else if(this->indexWP == 0)
      this->directionWP = 1;
    this->indexWP += this->directionWP;

    std::list<math::Vector3>::iterator it = this->listWP.begin();
    std::advance(it, this->indexWP);
    math::Vector3 wp = *it;
    
    //std::cout << "wp: " << wp << " idx: " << this->indexWP << " size: " << this->listWP.size() << std::endl;
...
void DRCVehiclePlugin::UpdateStates()
{
  ...
  this->linVel = this->model->GetRelativeLinearVel();
  this->angVel = this->model->GetRelativeAngularVel();
  this->pose = this->model->GetWorldPose();
  math::Vector3 position = math::Vector3(pose.pos.x, pose.pos.y, 0.0);
  math::Quaternion rot = pose.rot;

  this->speedState = linVel.x;
  this->headingState = atan2(2*(rot.w*rot.z+rot.x*rot.y), 1-2*(rot.y*rot.y+rot.z*rot.z));

  common::Time curTime = this->world->GetSimTime();
  double dt = (curTime - this->lastTime).Double();

  double dist2WP = math::Vector3(position.x-waypointCmd.x, position.y-waypointCmd.y, 0).GetLength();

  if (dt > 0)
  {
    if(dist2WP < 2*this->cruiseSpeed)
    {
      if(this->prev_waypointCmd == this->waypointCmd) // this should only happen on the initial condition
      {
        this->prev_waypointCmd = position;
        this->prev_waypointCmd.z = 1.0;

        this->listWP.push_front(this->prev_waypointCmd); // add initial position to waypoint list

        this->waypointCmd = GetNextWaypoint();
        this->speedCmd = this->cruiseSpeed;
      }
      else
      {
        if(!this->loopWP && this->waypointCmd == this->listWP.front())
          this->cruiseSpeed = 0;
        else
        {
          this->prev_waypointCmd = this->waypointCmd;
          this->waypointCmd = GetNextWaypoint();
        }
      }
    }

    // PID cross track and heading control throught steering control
    math::Vector3 AB = this->waypointCmd - this->prev_waypointCmd;
    math::Vector3 AP = position - this->prev_waypointCmd; AP.z = 0.0;
    math::Vector3 QP = AB.Cross(AP)/AB.GetLength();
    this->crossTrackState = QP.z;
    this->crossTrackError = this->crossTrackPID.Update(crossTrackState, dt); // crossTrackCmd = 0

    this->headingCmd = atan2(AB.y, AB.x);
    this->headingError = this->headingState - this->headingCmd;
    if(this->headingError > M_PI)
      this->headingError -= 2*M_PI;
    if(this->headingError < -M_PI)
      this->headingError += 2*M_PI;
    
    double headingError_ = this->headingPID.Update(this->headingError, dt);
    this->handWheelCmd = this->crossTrackError + headingError_;
    ...
    // PID speed control
    double speedError = this->speedState - this->cruiseSpeed;
    this->gasPedalCmd = this->speedCtrlPID.Update(speedError, dt);
    ...
    // \TODO correct PID for FNR button to make it stop switching from F<->R
    ...
    this->fnrSwitchJoint->SetForce(0, fnrSwitchCmd);//fnrSwitchPIDCmd);
