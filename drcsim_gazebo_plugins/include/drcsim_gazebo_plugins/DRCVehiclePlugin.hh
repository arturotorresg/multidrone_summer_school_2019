...
namespace gazebo
{
  ...
  class DRCVehiclePlugin : public ModelPlugin
  {
    ...
    /// \param[in] _speedPosition speed controller position in m/s
    ...
    /// \brief Returns the next waypoint by managing the WP list correctly
    math::Vector3 GetNextWaypoint();

    /// \brief Returns the current pose of the vehicle
    math::Pose GetPose();

    /// \brief Returns the current linear velocity
    math::Vector3 GetLinVel();

    /// \brief Returns the current angular velocity
    math::Vector3 GetAngVel();

    /// \brief Sets the next waypoint where we shall point to
    /// \param[in] _waypoint Desired waypoint
    public: void SetWaypointState(math::Vector3 _waypoint);
    ...
    /// \brief Return the vehicle speed
    public: double GetSpeedState();

    /// \brief Returns the vehicle heading
    public: double GetHeadingState();

    /// \brief Returns the vehicle heading error
    public: double GetHeadingError();

    /// \brief Returns the vehicle cross track
    public: double GetCrossTrackState();

    /// \brief Returns the vehicle cross track error
    public: double GetCrossTrackError();

    /// \brief Set the speed value
    /// \param[in] _speed Speed in m/s
    public: void SetSpeedState(double _speed);
    ...
    protected: double speedCmd;
    protected: double cruiseSpeed;
    protected: double cruseSpeedCmd;
    protected: double headingCmd;
    protected: math::Vector3 waypointCmd;
    protected: math::Vector3 prev_waypointCmd;
    protected: std::list<math::Vector3> listWP;
    protected: bool loopWP;
    protected: int indexWP;
    protected: int directionWP;
    ...
    private: common::PID speedCtrlPID;
    private: common::PID headingPID;
    private: common::PID crossTrackPID;
    ...
    private: math::Vector3 linVel;
    private: math::Vector3 angVel;
    ...
    private: double speedState;
    private: double headingState;
    private: double headingError;
    private: double crossTrackState;
    ...
    private: double speedCtrlPgain;
    private: double speedCtrlIgain;
    private: double speedCtrlDgain;
    private: double headingPgain;
    private: double headingIgain;
    private: double headingDgain;
    private: double crossTrackPgain;
    private: double crossTrackIgain;
    private: double crossTrackDgain;
  };
/// \}
}
#endif
